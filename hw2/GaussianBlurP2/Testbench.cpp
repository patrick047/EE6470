...
void Testbench::do_GaussianBlur()
{
  ...
  for (y = 0; y < height; ++y)
  {
    for (x = 0; x < width; ++x)
    {
      o_r.write(*(source_bitmap + bytes_per_pixel * (width * y + x + offset) + 2));
      o_g.write(*(source_bitmap + bytes_per_pixel * (width * y + x + offset) + 1));
      o_b.write(*(source_bitmap + bytes_per_pixel * (width * y + x + offset) + 0));
      pixels += 3;
    }
  }
...
void Testbench::do_GaussianBlur_out()
{
  ...
  for (int y = 0; y != height; y++)
  {
    for (int x = 0; x != width; x++)
    {
      ...
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = rr;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = gg;
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = bb;
      std::cout << std::setw(12) << rr << std::setw(12) << gg << std::setw(12) << bb << std::endl;
      wait();
    }
  }
  //sc_stop();
